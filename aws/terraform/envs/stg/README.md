...
- **AWS Profile** configured: `myproject-stg`
- **S3 Bucket** cho Terraform state: `myproject-stg-iac-state`
- **DynamoDB Table** cho state lock: `myproject-stg-terraform-state-lock`
...
aws configure --profile myproject-stg
...
aws s3 ls | grep myproject-stg-iac-state
aws dynamodb list-tables | grep myproject-stg-terraform-state-lock
```
...
cd ../terraform/envs/stg/3.order-processing
cp ../../../../terraform-dependencies/lambda-function/*.zip ./

# C2: chạy script
cd aws
chmod +x build-lambda-packages.sh
./build-lambda-packages.sh stg order-processing
...
```bash
# Initialize Terraform
make init e=stg s=order-processing

# Review plan
make plan e=stg s=order-processing

# Apply infrastructure
make apply e=stg s=order-processing
...
cd aws/terraform/envs/stg/3.order-processing
...
aws secretsmanager put-secret-value \
    --secret-id myproject-stg-email-config \
    --secret-string '{
...
aws logs tail /aws/lambda/myproject-stg-webhook-handler \
    --follow \
    --profile myproject-stg
```
...
File: [`terraform.stg.tfvars`](aws/terraform/envs/stg/terraform.stg.tfvars)

```hcl
project = "myproject"     # Project name prefix
env     = "stg"           # Environment name
...
**Access URL:**
```
https://ap-northeast-1.console.aws.amazon.com/cloudwatch/home?region=ap-northeast-1#dashboards:name=myproject-stg-order-processing
```
...
**Log Groups:**
```
/aws/lambda/myproject-stg-webhook-handler
/aws/lambda/myproject-stg-email-processor
/aws/lambda/myproject-stg-inventory-processor
/aws/lambda/myproject-stg-database-processor
/aws/apigateway/myproject-stg-webhook-api
```
...
aws lambda get-function \
    --function-name myproject-stg-webhook-handler \
    --profile myproject-stg \
    --region ap-northeast-1
...
aws cloudwatch get-metric-statistics \
    ...
    --dimensions Name=TableName,Value=myproject-stg-orders \
...
5. **Deploy Staging** (`.github/workflows/deploy-stg.yaml`)
   - Full deployment pipeline for stg environment
...
aws logs tail /aws/lambda/myproject-stg-webhook-handler --follow
```
...
# Edit aws/terraform/envs/stg/3.order-processing/lambda.tf
...
```bash
# Get DLQ URL
aws sqs list-queues --queue-name-prefix myproject-stg-email-dlq
...
aws logs filter-log-events \
    --log-group-name /aws/lambda/myproject-stg-email-processor \
...
aws dynamodb describe-table \
    --table-name myproject-stg-orders \
...
# Edit aws/terraform/envs/stg/3.order-processing/dynamodb.tf
...
aws dynamodb get-item \
    --table-name myproject-stg-terraform-state-lock \
    --key '{"LockID":{"S":"myproject-stg-iac-state/3.order-processing/terraform.stg.tfstate-md5"}}'
...
aws logs filter-log-events \
    --log-group-name /aws/lambda/myproject-stg-webhook-handler \
    --filter-pattern "ERROR" \
...
aws cloudwatch get-metric-statistics \
    ...
    --dimensions Name=FunctionName,Value=myproject-stg-webhook-handler \
...
aws logs tail /aws/apigateway/myproject-stg-webhook-api --follow
```
...
4. **Scheduled Health Checks**:
   ...
   aws events put-rule \
       --name myproject-stg-health-check \
...
aws dynamodb restore-table-from-backup \
    --target-table-name myproject-stg-orders-restored \
    --backup-arn arn:aws:dynamodb:...:backup/...

# 2. Restore Terraform state
aws s3 cp s3://myproject-stg-iac-state/... \
